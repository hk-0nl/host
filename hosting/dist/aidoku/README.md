...
- Gelbooru v17: `https://raw.githubusercontent.com/hk-0nl/host/main/hosting/dist/aidoku/sources/multi.gelbooru-v17.aix`
...
Gelbooru v17 adds a distinct Post Family section with native Aidoku links for explicit parents, direct children, and siblings derived from their shared parent. Signed-out detail HTML supplies numeric parent tags, and bounded cached `parent:<id>` listings supply children and siblings without depending on Gelbooru's unreliable `data-has-children` flag. Pool Relationships and Similar Posts remain separate description sections; multiple pools also remain native `pool:<id> - <name>` tags for in-app browsing. Chapter 2 is reserved for post data, Translations, and Comments. Account-backed Favorites/Saved Searches, account mutations, and pool name-to-ID discovery remain planned.
