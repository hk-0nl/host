...
- Gelbooru v9: `https://raw.githubusercontent.com/hk-0nl/host/main/hosting/dist/aidoku/sources/multi.gelbooru-v9.aix`
...
Gelbooru v9 provides signed-out search/filters, Latest and all-time Overall/Static/Animated discovery, Top Tags Home links, categorized metadata, static/GIF image pages, and explicit WebM/MP4 web fallback because Aidoku has no native video page type. Display All now reconciles both enabled and disabled states through Gelbooru's anonymous session-bound options form; no login is required. Website authentication uses Gelbooru Web Login; DAPI User ID/API key uses a test-and-save login row. Authentication failures fall back to public browsing. Typed Favorites and Saved Searches listing boundaries remain visible read-only placeholders for future account work.
