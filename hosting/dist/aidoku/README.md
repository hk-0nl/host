...
- Gelbooru v8: `https://raw.githubusercontent.com/hk-0nl/host/main/hosting/dist/aidoku/sources/multi.gelbooru-v8.aix`
...
Gelbooru v8 provides signed-out search/filters, Latest and all-time Overall/Static/Animated discovery, Top Tags Home links, categorized metadata, static/GIF image pages, and explicit WebM/MP4 web fallback because Aidoku has no native video page type. Display All uses Gelbooru's verified session-bound preference and may deterministically reorder broad score-ranked listings. Website authentication uses Gelbooru Web Login; DAPI User ID/API key uses a test-and-save login row. Authentication failures fall back to public browsing. Favorites and Saved Searches are visible read-only placeholders for future account work.
