...
- E-Hentai v7: `https://raw.githubusercontent.com/hk-0nl/host/main/hosting/dist/aidoku/sources/multi.ehentai-v7.aix`
...
E-Hentai v7 corrects split-gallery Source Order so pages 1-250 are Chapter 1, derives saved-search utilities from effective text, tag, creator, uploader, title, and language queries, and returns a dedicated result item for every favorite action. Favorite changes now inspect the existing provider state, preserve notes, choose add versus update semantics, verify the resulting favorite state, and expose the last result in settings. Cold split chapters fetch and persist only the requested page range instead of crawling the complete gallery; a live cold pages 1-250 probe on a 1,788-page gallery completed in about four seconds of source runtime. Existing complete v6 manifests remain reusable. Range caches are bounded to 24 entries, while parallel manifest loading and reader splitting remain independent toggles. Eleven serialized tests and all Aidoku package/schema gates pass.
