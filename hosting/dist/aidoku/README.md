...
- Gelbooru v16: `https://raw.githubusercontent.com/hk-0nl/host/main/hosting/dist/aidoku/sources/multi.gelbooru-v16.aix`
...
Gelbooru v16 uses one canonical relationship surface: the detail description contains named Pool Relationships links and native-clickable Similar Posts, while chapter 2 is reserved for post data, Translations, and Comments. Multiple pools also appear at the end of native tags as `pool:<id> - <name>`; tapping one searches the provider's numeric pool ID. The description's pool link preserves access to the provider's ordered pool page, while the tag provides native Aidoku browsing. Gelbooru does not accept pool-name post searches, so name-to-ID discovery remains planned. Parent/child/sibling family relationships, authenticated Favorites/Saved Searches, and account mutations remain unimplemented.
