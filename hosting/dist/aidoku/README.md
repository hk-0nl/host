...
- Gelbooru v25: `https://raw.githubusercontent.com/hk-0nl/host/main/hosting/dist/aidoku/sources/multi.gelbooru-v25.aix`
...
Gelbooru v25 adds a guarded Add Favorite action to post details when a complete Web Login session is configured. The action uses Gelbooru's first-party favorite endpoint, returns to the same Aidoku post, and displays the provider result there; it does not vote on the post. Favorite removal and saved-search mutations remain deferred pending authenticated response captures. Existing Favorites, Saved Searches, native tag and relationship navigation, companion metadata, translations, comments, and readable-tag behavior remain intact.
