...
- E-Hentai v5: `https://raw.githubusercontent.com/hk-0nl/host/main/hosting/dist/aidoku/sources/multi.ehentai-v5.aix`
...
E-Hentai v5 keeps the v4 account, discovery, metadata, and strict image-response handling while making large galleries substantially lighter to open. It builds the ordered page manifest from the Lo-Fi gallery view, uses authenticated MPV metadata when available, retains the standard gallery crawl as a fallback, and applies one bounded retry plus cycle/size guards. The source no longer forces a viewer direction, so new galleries inherit Aidoku's global reading-mode setting. Eight serialized tests cover public discovery, Home, details, chapters, deep links, manifest ordering, MPV context construction, and direct animated-WebP resolution for all 158 pages of gallery 4055860 including pages 134/158. ExHentai, Favorites, igneous refresh, and privileged MPV remain account-gated. Aidoku's static/tinted animated-WebP rendering is an app decoder limitation and is not fixed by this source release.
