...
- Gelbooru v11: `https://raw.githubusercontent.com/hk-0nl/host/main/hosting/dist/aidoku/sources/multi.gelbooru-v11.aix`
...
Gelbooru v11 provides signed-out search/filters, Latest and all-time Overall/Static/Animated discovery, Top Tags Home links, categorized metadata, static/GIF image pages, and explicit WebM/MP4 web fallback because Aidoku has no native video page type. Display All OFF adds Gelbooru's documented loli/shota exclusions to both HTML and DAPI queries, while ON removes them; the website preference cookie remains a secondary compatibility path. The media item is chapter 1 so Aidoku's Read action opens content first. Chapter 2 is Post information and now includes provider-authored translation notes, including multiline text and region coordinates, when Gelbooru supplies them. Website authentication, Favorites, Saved Searches, comments, and relationship groups remain foundations or planned work rather than claimed account functionality.
