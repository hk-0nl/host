...
- Gelbooru v14: `https://raw.githubusercontent.com/hk-0nl/host/main/hosting/dist/aidoku/sources/multi.gelbooru-v14.aix`
...
Gelbooru v14 enriches the six provider-supplied Similar Posts with cached title tags, rating, score, and resolution. Detail-description links use a package-unique hosted deep-link prefix so Aidoku selects this Gelbooru source and opens the related post natively; chapter-2 copies remain plain summaries because stock Aidoku's text reader opens HTTP links externally. Multiple Pool Relationships remain independent, also appear as native `pool:<numeric id>` tags, and can be browsed with the Pool ID filter. Gelbooru does not accept pool-name post searches, so name-to-ID discovery remains planned. Parent/child/sibling family relationships, authenticated Favorites/Saved Searches, and account mutations remain unimplemented.
