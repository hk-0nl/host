...
- Gelbooru v38: `https://raw.githubusercontent.com/hk-0nl/host/main/hosting/dist/aidoku/sources/multi.gelbooru-v38.aix`
...
Gelbooru v38 provides signed-out search/filters, Latest and all-time Overall/Static/Animated discovery, Top Tags Home links, categorized metadata, static/GIF image pages, family browsing, and explicit WebM/MP4 web fallback because Aidoku has no native video page type. Display All OFF adds Gelbooru's documented loli/shota exclusions to both HTML and DAPI queries, while ON removes them; the website preference cookie remains a secondary compatibility path. The media item is chapter 1 so Aidoku's Read action opens content first. Chapter 2 is Post information and includes provider-authored translation notes, public comments, relationship links, and account-action status when Gelbooru supplies them. Saved searches, favorites, readable-tag modes, optional family-as-chapters, and cache controls are exposed through source settings; account mutations still require a valid Gelbooru session or DAPI credential and are reported as provider results rather than claimed offline functionality.
