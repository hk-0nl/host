...
- Gelbooru v19: `https://raw.githubusercontent.com/hk-0nl/host/main/hosting/dist/aidoku/sources/multi.gelbooru-v19.aix`
...
Gelbooru v19 fixes v18's native tag-click regression: Aidoku's `genre` Select values now become real provider queries for ordinary, artist, metadata, and pool labels instead of falling back to Latest. The primary Pool Relationships description link opens a native dynamic `pool:<id>` listing; an adjacent ordered-pool-page link retains Gelbooru's provider sequence view. Parent, Children, and Siblings remain whitespace-separated native subsections without a Post Family umbrella heading, Similar Posts remains independent, and chapter 2 remains reserved for post data, Translations, and Comments. Alias/autocomplete semantics, pool name-to-ID discovery, and account-backed features remain planned.
