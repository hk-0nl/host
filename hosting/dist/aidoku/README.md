...
- Gelbooru v15: `https://raw.githubusercontent.com/hk-0nl/host/main/hosting/dist/aidoku/sources/multi.gelbooru-v15.aix`
...
Gelbooru v15 enriches the six provider-supplied Similar Posts with cached title tags, rating, score, and resolution. Confirmed-working detail-description links use a package-unique hosted deep-link prefix so Aidoku selects this Gelbooru source and opens the related post natively; the non-linking duplicate was removed from chapter 2. Post information now prioritizes Translations and Comments before Pool Relationships. Multiple pools retain named provider links and appear at the end of native tags as `pool:<id> - <name>`; tapping one searches the provider's numeric pool ID. Gelbooru does not accept pool-name post searches, so name-to-ID discovery remains planned. Parent/child/sibling family relationships, authenticated Favorites/Saved Searches, and account mutations remain unimplemented.
