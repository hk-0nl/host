...
class DefaultExtension extends MProvider {
    ...
    async getPopular(page) {
        ...
        for (const book of books) {
            ...
            if (!bookRelPath || bookRelPath === "#") continue;
            const bookTitle = book.selectFirst("b")?.text?.trim() || anchor.text.trim();
            const bookLink = bookRelPath.startsWith("http") ? bookRelPath : baseUrl + bookRelPath;
            // Image is already in the listing — no extra fetch needed
            const img = book.selectFirst("img");
            const imageUrl = img?.attr("src") || "";
            if (bookTitle) bookList.push({ name: bookTitle, link: bookLink, imageUrl });
        }
    ...
    async getLatestUpdates(page) {
        ...
        for (const book of books) {
            ...
            if (bookTitle === "View More>>") continue;
            const bookLink = bookRelPath.startsWith("http") ? bookRelPath : baseUrl + bookRelPath;
            // Latest list has no inline images — use a placeholder (no per-book fetch)
            bookList.push({ name: bookTitle, link: bookLink, imageUrl: "" });
        }
        return { list: bookList, hasNextPage: false };
    }
