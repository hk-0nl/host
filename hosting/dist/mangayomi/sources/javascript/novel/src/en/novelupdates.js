...
class DefaultExtension extends MProvider {
  ...
  getManualAuthMessage() {
    return "Novel Updates blocked the request. On iOS, the localhost Cloudflare helper is often unavailable. Open Novel Updates in Mangayomi webview and complete the challenge, or paste a valid cookie_header and matching user_agent in source settings.";
  }
  ...
  async requestPage(url) {
    let res;
    try {
      res = await new Client().get(url, this.buildHeaders(url));
    } catch (e) {
      const msg = e.toString();
      if (msg.includes("Connection refused") || msg.includes("SocketException")) {
        throw new Error("Connection refused while contacting the local Cloudflare helper on localhost. This usually means the helper is unavailable on this device or platform. " + this.getManualAuthMessage());
      }
      throw e;
    }
  ...
  async requestForm(url, body) {
    ...
    let res;
    try {
      res = await new Client().post(url, headers, body);
    } catch (e) {
      const msg = e.toString();
      if (msg.includes("Connection refused") || msg.includes("SocketException")) {
        throw new Error("Connection refused while contacting the local Cloudflare helper on localhost. This usually means the helper is unavailable on this device or platform. " + this.getManualAuthMessage());
