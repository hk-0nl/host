...
class DefaultExtension extends MProvider {
    ...
    getHeaders(url) {
        return {
            "User-Agent": this.getPreference("custom_user_agent") || "Dalvik/2.1.0 (Linux; U; Android 14; 22081212UG Build/UKQ1.230917.001)",
            "Connection": "close"
        };
    }
    async getPopular(page) {
        const offset = 20 * (page - 1);
        const translatedLang = (this.source.lang && this.source.lang !== "all") ? `&availableTranslatedLanguage[]=${this.source.lang}` : "";
        const url = `${this.source.apiUrl}/manga?limit=20&offset=${offset}${translatedLang}&includes[]=cover_art&contentRating[]=safe&contentRating[]=suggestive${this.preferenceOriginalLanguages()}&order[followedCount]=desc`;
        const response = await this.client.get(url, this.getHeaders());
        return this.mangaRes(response.body);
    }
    async getLatestUpdates(page) {
        const offset = 20 * (page - 1);
        const translatedLang = (this.source.lang && this.source.lang !== "all") ? `&translatedLanguage[]=${this.source.lang}` : "";
        const url = `${this.source.apiUrl}/chapter?limit=20&offset=${offset}${translatedLang}&includeFutureUpdates=0&order[publishAt]=desc&includeFuturePublishAt=0&includeEmptyPages=0`;
        const response = await this.client.get(url, this.getHeaders());
    ...
    async search(query, page, filters) {
        ...
        let url = `${this.source.apiUrl}/manga?includes[]=cover_art&offset=${offset}&limit=20`;
        if (query) {
            url += `&title=${encodeURIComponent(query)}`;
        }
        if (this.source.lang && this.source.lang !== "all") {
            url += `&availableTranslatedLanguage[]=${this.source.lang}`;
        }
    ...
    async getDetail(url) {
        ...
        const manga = {};
        const coverRel = data.relationships ? data.relationships.find(rel => rel.type === "cover_art") : null;
        ...
        const authors = data.relationships
            ? data.relationships.filter(rel => rel.type === "author").map(rel => rel.attributes.name)
            : [];
        manga.author = authors.join(", ");
        manga.description = data.attributes?.description ? (data.attributes.description[this.source.lang] ?? data.attributes.description.en ?? "") : "";
        manga.genre = data.attributes?.tags ? data.attributes.tags.map(tag => tag.attributes.name.en) : [];
        ...
        manga.status = { "ongoing": 0, "completed": 1, "hiatus": 2, "cancelled": 3 }[data.attributes?.status] ?? 0;
    ...
    async fetchPaginatedChapters(mangaId, lang) {
        ...
        const translatedLang = (lang && lang !== "all") ? `&translatedLanguage[]=${lang}` : "";

        while (hasMoreResults) {
            const url = `${this.source.apiUrl}/manga/${mangaId}/feed?limit=500&offset=${offset}&includes[]=user&includes[]=scanlation_group&order[volume]=desc&order[chapter]=desc${translatedLang}&includeFuturePublishAt=0&includeEmptyPages=0&contentRating[]=safe&contentRating[]=suggestive&contentRating[]=erotica&contentRating[]=pornographic`;
    ...
    extractChapters(paginatedData) {
        ...
        const dataList = Array.isArray(paginatedData?.data) ? paginatedData.data : [];
        ...
        return [
            ...
            },
            {
                ...
                state: [
                    ...
                    ["English (Comic)", "originalLanguage[]=en"],
                    ["Chinese (Manhua)",
                        "originalLanguage[]=zh&originalLanguage[]=zh-hk"],
            ...
            },
            {
                ...
                ].map((x, index) => ({ type_name: 'CheckBox', name: x[0], value: x[1], state: index < 2 }))
            },
            {
                type_name: "GroupFilter",
