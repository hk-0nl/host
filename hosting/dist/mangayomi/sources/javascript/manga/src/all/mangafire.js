...
class DefaultExtension extends MProvider {
    ...
    async getDetail(url) {
        ...
        for (let i = 0; i < ids.length; i++) {
            ...
            const name = chapterId.text;
            const mangaId = chapterId.attr("data-id");

            let scanlator = null;
            let dateUpload = null;
            if (viewType == "chapter") {
                const chapElement = chapElements[i];
                const title = chapElement.selectFirst("a").attr("title").split(" - ");
            ...
            const readUrl = this.source.baseUrl + `/ajax/read/${viewType}/${mangaId}`;
            detail.chapters.push({ name, url: readUrl, dateUpload, scanlator });
        }
        return detail;
    }
    ...
    async getPageList(url) {
        ...
        const pages = [];
        const hdr = { "Referer": this.source.baseUrl };
