class DefaultExtension extends MProvider {
            ...
            .map(media => {
                ...
                anime.name = (() => {
                    ...
                    switch (preferenceTitle) {
                        case "romaji":  return media?.title?.romaji || "";
                        case "english": return media?.title?.english?.trim() || media?.title?.romaji || "";
                        case "native":  return media?.title?.native || "";
                        default:        return media?.title?.romaji || "";
                    }
                })();
    ...
    async getDetail(url) {
        ...
        anime.status = (() => {
            switch (media?.status) {
                ...
                case "NOT_YET_RELEASED":return 3;
                default:                return 5;
            }
        })();
        ...
        anime.author = (media?.studios?.nodes?.map(n => n.name).filter(Boolean) || []).sort().join(", ");

        // ani.zip mapping for Kitsu ID
        const response = await this.client.get(
            `https://api.ani.zip/mappings?anilist_id=${url}`,
            { "Connection": "close" }
        );
        ...
        if (kitsuId) {
            if (media?.format === "MOVIE") {
                anime.episodes = [{ url: `/stream/movie/${kitsuId}.json`, name: "Movie" }];
            } else {
                ...
                for (let i = 1; i <= epCount; i++) {
                    episodeList.push({ url: `/stream/series/kitsu:${kitsuId}:${i}.json`, name: `Episode ${i}` });
                }
                anime.episodes = episodeList.reverse();
            }
        }
    ...
    appendQueryParam(key, values) {
        ...
        const filtered = Array.from(values).filter(v => v && v.trim() !== "").join(",");
    ...
    async getVideoList(url) {
        ...
        // Build Torrentio URL entirely inside a class method — this.source is valid here
        let mainURL = `${this.source.baseUrl}/`;
        mainURL += this.appendQueryParam("providers",    prefs.get("provider_selection"));
        mainURL += this.appendQueryParam("language",     prefs.get("lang_selection"));
        mainURL += this.appendQueryParam("qualityfilter",prefs.get("quality_selection"));
        mainURL += this.appendQueryParam("sort",         new Set([prefs.get("sorting_link")]));
        mainURL += url;
        mainURL = mainURL.replace(/\|$/, "");

        const responseEpisodes = await this.client.get(mainURL, { "Connection": "close" });
        ...
        const animeTrackers = [
            ...
            "udp://opentrackr.i2p.rocks:6969/announce",
        ...
        const videos = this.sortVideos((streamList.streams || []).map(stream => {
            ...
            return { url: hash, originalUrl: hash, quality: videoTitle };
        }));

        const numberOfLinks = prefs.get("number_of_links");
        return numberOfLinks === "all" ? videos : videos.slice(0, parseInt(numberOfLinks));
    }

    sortVideos(videos) {
        const prefs = new SharedPreferences();
        const isDub       = prefs.get("dubbed");
        const isEfficient = prefs.get("efficient");
        ...
        return videos.sort((a, b) => {
            ...
            return regexMatchA - regexMatchB || isDubA - isDubB || isEfficientA - isEfficientB;
        });
    }
        ...
        return [
            ...
            },
            {
                ...
                "multiSelectListPreference": {
                    ...
                    "summary": "",
                    "entries": ["YTS","EZTV","RARBG","1337x","ThePirateBay","KickassTorrents","TorrentGalaxy","MagnetDL","HorribleSubs","NyaaSi","TokyoTosho","AniDex","🇷🇺 Rutor","🇷🇺 Rutracker","🇵🇹 Comando","🇵🇹 BluDV","🇫🇷 Torrent9","🇪🇸 MejorTorrent","🇲🇽 Cinecalidad"],
                    "entryValues": ["yts","eztv","rarbg","1337x","thepiratebay","kickasstorrents","torrentgalaxy","magnetdl","horriblesubs","nyaasi","tokyotosho","anidex","rutor","rutracker","comando","bludv","torrent9","mejortorrent","cinecalidad"],
                    "values": ["nyaasi"]
                }
            },
            {
                ...
                "multiSelectListPreference": {
                    ...
                    "summary": "",
                    "entries": ["BluRay REMUX","HDR/HDR10+/Dolby Vision","Dolby Vision","4k","1080p","720p","480p","Other (DVDRip/HDRip/BDRip...)","Screener","Cam","Unknown"],
                    "entryValues": ["brremux","hdrall","dolbyvision","4k","1080p","720p","480p","other","scr","cam","unknown"],
                    "values": ["720p","480p","other","scr","cam","unknown"]
                }
            },
            {
                "key": "lang_selection",
            ...
            },
            {
                ...
                "listPreference": {
                    ...
                    "summary": "",
                    "valueIndex": 0,
                    "entries":     ["By quality then seeders","By quality then size","By seeders","By size"],
...
// Single mangayomiSources declaration — the stale duplicate at top of file was removed (Task F fix)
const mangayomiSources = [{
    "name": "Torrentio Anime",
    "lang": "all",
    "baseUrl": "https://torrentio.strem.fun",
    "apiUrl": "",
    "iconUrl": "https://torrentio.strem.fun/manifest.json",
    "typeSource": "torrent",
    "itemType": 1,
    "version": "1.0.1",
    ...
    "isNsfw": false,
    "hasCloudflare": false,
    "sourceCodeUrl": "https://raw.githubusercontent.com/hk-0nl/host/main/hosting/dist/mangayomi/anime/src/all/torrentioanime.js"
}];
