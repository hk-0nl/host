...
class ArchiveOfOurOwn extends MProvider {
  ...
  Future<MPages> getPopular(int page) async {
    ...
    return _fetchListing(page: page, sortColumn: 'kudos_count');
  }

  @override
  Future<MPages> getLatestUpdates(int page) async {
    return _fetchListing(page: page, sortColumn: 'revised_at');
  }
  ...
  Future<MManga> getDetail(String url) async {
    ...
    final document = parseHtml(await _fetchWithBackoff(fullUrl));

    final manga = MManga();
    manga.name =
        document.selectFirst('h2.title.heading')?.text.trim() ??
        'Work ' + workId;
    manga.author =
        document.selectFirst('.byline.heading a[rel="author"]')?.text.trim() ??
        '';
    manga.description =
        document
    ...
    final chapCounter = document.selectFirst('dd.chapters')?.text.trim() ?? '';
    ...
    if (options.isNotEmpty) {
      manga.chapters = options.asMap().entries.map((entry) {
        ...
        chapter.name = label.isEmpty
            ? 'Chapter ' + (idx + 1).toString()
            : label;
        chapter.url = chapUrl;
        return chapter;
      }).toList();
  ...
  Future<List<dynamic>> getPageList(String url) async {
    final document = parseHtml(await _fetchWithBackoff(url));
  ...
  Future<MPages> _fetchListing({
    ...
    final tagId =
        (getPreferenceValue(source.id, 'ao3_fandom_tag')?.toString() ?? '')
            .trim();
    ...
    return _parseListing(await _fetchWithBackoff(uri.toString()));
  }
  ...
  Map<String, String> _headers() {
    return {
      ...
      'Connection': 'close',  // Phase 9 global hardening
    };
  }
