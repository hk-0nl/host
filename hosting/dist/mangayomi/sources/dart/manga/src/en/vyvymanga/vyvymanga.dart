...
class VyvyManga extends MProvider {
  ...
  Future<MPages> search(String query, int page, FilterList filterList) async {
    ...
    String url =
        "${_base()}/?s=${Uri.encodeQueryComponent(query)}&post_type=wp-manga&page=$page";
  ...
  Future<MManga> getDetail(String url) async {
    ...
    manga.name =
        document
            .selectFirst("div.post-title h1, h1.manga-title")
            ?.text
            .trim() ??
        "";
    ...
    if (manga.description.isEmpty) {
      manga.description =
          document
              .selectFirst("div.summary__content, div.manga-description")
              ?.text
              .trim() ??
          "";
    }
    ...
    manga.imageUrl =
        document.selectFirst("div.summary_image img")?.attr("data-src") ??
            ...
            .map(
              (src) => {
                ...
                "headers": {
                  "Referer": _base() + "/",
                  "User-Agent":
                      "Mozilla/5.0 (compatible; Mangayomi-VyvyManga/1.0)",
                  "Connection": "close",
                },
              },
            )
  ...
  Future<String> _fetchChapterList(
    String mangaUrl,
  ...
  MPages _parseListingPage(String html) {
    ...
    for (final el in document.select(
      ...
      final title =
          el.selectFirst("h3.h5, span.font-title, .post-title")?.text.trim() ??
    ...
    final hasNext =
        html.contains('class="next page-numbers"') || html.contains("nav-next");
  ...
  MPages _parseSearchPage(String html) {
    ...
    for (final el in document.select(
      "div.c-tabs-item__content, div.row.c-tabs-item",
    )) {
      final anchor = el.selectFirst("a");
      if (anchor == null) continue;
      final link = anchor.attr("href").trim();
      ...
      final title =
          el.selectFirst("h3, .post-title")?.text.trim() ??
  ...
  List<MChapter> _parseChapterList(String html, String mangaUrl) {
    ...
    for (final el in document.select("li.wp-manga-chapter")) {
      ...
      chapters.add(MChapter(name: name, url: url, dateUpload: date));
    }
    return chapters;
  }
