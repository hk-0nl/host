...
class Viz extends MProvider {
  ...
  Future<MManga> getDetail(String url) async {
    ...
    if (apiData != null && apiData.isNotEmpty) {
      ...
      manga.name =
          document.selectFirst("h2.type-sm, .series-title")?.text.trim() ??
          slug;
      manga.description =
          document
              .selectFirst(".series-intro--desc, .o_synopsis")
              ?.text
              .trim() ??
          "";
      manga.imageUrl =
  ...
  Future<MPages> _fetchCatalog(
    ...
    if (query.isNotEmpty)
      apiUrl += "&search=${Uri.encodeQueryComponent(query)}";
    ...
    if (query.isNotEmpty)
      htmlUrl += "?search=${Uri.encodeQueryComponent(query)}&page=$page";
    else
      htmlUrl += "?page=$page";
  ...
  MPages _parseHtmlCatalog(MDocument doc, int page) {
    ...
    for (final el in doc.select("div.product-item, article.series-item")) {
      ...
      final name =
          el
              .selectFirst("div.title, .series-title, img")
              ?.attr("alt")
              ?.trim() ??
      ...
      final cover =
          el
              .selectFirst("img[data-original], img.lazy")
              ?.attr("data-original") ??
    ...
    final hasNext =
        doc.selectFirst(".o_next_btn, a[aria-label='Next']") != null;
    return MPages(items, hasNext);
  }
  ...
  List<MChapter> _parseApiChapters(dynamic raw, String mangaUrl) {
    ...
    for (final c in raw) {
      ...
      final date =
          ch["date"]?.toString() ?? ch["releaseDate"]?.toString() ?? "";
  ...
  List<MChapter> _parseHtmlChapters(MDocument doc, String mangaUrl) {
    ...
    for (final el in doc.select("div.o_chapter, li.chapter-list-item")) {
      ...
      final name =
          el.selectFirst(".chapter-title, span.ch-num")?.text.trim() ??
          anchor?.text.trim() ??
          "";
