...
class Kmanga extends MProvider {
  ...
  Future<MManga> getDetail(String url) async {
    ...
    manga.description = descParagraphs.isNotEmpty
        ? descParagraphs
        : document
                  .selectFirst("div.summary__content, div.manga-excerpt")
                  ?.text
                  .trim() ??
              "";

    // Cover image — try data-src (lazy-loaded) first, then src
    final imgEl = document.selectFirst(
      "div.summary_image img, .manga-thumbnail img",
    );
            ...
            .map(
              (src) => {
                ...
                "headers": {
                  "Referer": url,
                  "User-Agent":
                      "Mozilla/5.0 (compatible; Mangayomi-Kmanga/1.0)",
                  "Connection": "close",
                },
              },
            )
  ...
  MPages _parseListing(String html) {
    ...
    for (final el in document.select(
      ...
      final name =
          el.selectFirst("h3.h5, span.manga-title, a[title]")?.text.trim() ??
  ...
  List<dynamic> getSourcePreferences() {
    return [
      EditTextPreference(
        ...
        title: "Base URL",
        summary:
            "Default: https://kmanga.nettruyenplus.com\n"
