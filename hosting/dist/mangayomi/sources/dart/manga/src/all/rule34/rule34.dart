  ...
  Future<MManga> getDetail(String url) async {
    ...
    if (isVideo) {
      ...
      if (fileUrl.isNotEmpty) {
        final isGif = ext == "gif";
        chapters.add(
          MChapter(name: isGif ? "Animated GIF" : "Image .$ext", url: fileUrl),
        );
      }
      if (chapters.isEmpty) {
        chapters.add(
  ...
  Future<MPages> _fetchPage(
    ...
    if (_blacklistRaw.isNotEmpty) {
      ...
      if (_exclusions.isNotEmpty) {
        extraTags = extraTags.isNotEmpty
            ? "$extraTags $_exclusions"
            : _exclusions;
      }
    }
    String params = "page=dapi&s=post&q=index&json=1&limit=20&pid=$pid";
    if (extraTags.isNotEmpty)
      params += "&tags=${Uri.encodeQueryComponent(extraTags)}";
  ...
  List<dynamic> getSourcePreferences() {
    return [
      ...
      EditTextPreference(
        ...
        summary:
            "Rule34 API now mandates authentication. Find your ID at rule34.xxx/index.php?page=account&s=options",
        value: "",
        dialogTitle: "User ID",
        dialogMessage: "",
      ),
