...
class MultiBooru extends MProvider {
  ...
  Future<MPages> _fetchPage(
    ...
    if (_blacklistRaw.isNotEmpty) {
      ...
      if (_exclusions.isNotEmpty) {
        _allTags = _allTags.isNotEmpty
            ? _allTags + " " + _exclusions
            : _exclusions;
      }
    }
    ...
    for (final postRaw in posts) {
      ...
      manga.link = _buildSinglePostUri(
        isDanbooru: isDan,
        id: post.id,
      ).toString();
  ...
  String _buildTagQuery({
    ...
    final sort = isDanbooru
        ? (sortIndex >= 0 && sortIndex < sortDan.length
              ? sortDan[sortIndex]
              : '')
        : (sortIndex >= 0 && sortIndex < sortGel.length
              ? sortGel[sortIndex]
              : '');
  ...
  Uri _buildSinglePostUri({required bool isDanbooru, required String id}) {
    final baseUrl = _getBaseUrl();
    final base = baseUrl.endsWith('/')
        ? baseUrl.substring(0, baseUrl.length - 1)
        : baseUrl;
  ...
  _BooruPost _normalizePost(
    Map<String, dynamic> data, {
    required bool isDanbooru,
  }) {
    final post = _BooruPost();
    post.id = data["id"]?.toString() ?? "";
    post.tags = isDanbooru
        ? (data["tag_string"]?.toString() ?? "")
        : (data["tags"]?.toString() ?? "");
    ...
    post.fileUrl =
        ...
        "";
    if (post.fileUrl.isEmpty &&
        !isDanbooru &&
        data.containsKey("directory") &&
  ...
  List<dynamic> getSourcePreferences() {
    return [
      EditTextPreference(
        ...
        title: "Base URL",
        summary:
            "The base domain for the Booru (e.g., https://danbooru.donmai.us or https://gelbooru.com)",
      ...
      ListPreference(
        ...
        summary:
            "Danbooru uses /posts.json; Gelbooru uses dapi. Hosts api.rule34.* use Gelbooru DAPI automatically.",
        valueIndex: 0,
      ...
      EditTextPreference(
        ...
        title: "AI Blocklist",
        summary:
            "Comma-separated tags to exclude from all results. Prepended as -tag exclusions on every request.",
