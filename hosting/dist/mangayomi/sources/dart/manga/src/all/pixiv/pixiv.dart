...
class Pixiv extends MProvider {
  ...
  Future<List<dynamic>> getPageList(String url) async {
    ...
    if (pageCount == 1) {
      ...
      imageUrls = metaPages
          .map(
            (p) =>
                ((p as Map)['image_urls'] as Map?)?['original']?.toString() ??
                '',
          )
  ...
  List<dynamic> getSourcePreferences() {
    return [
      EditTextPreference(
        ...
        dialogMessage:
            'Paste the refresh_token value printed by gppt or your '
  ...
  MPages _parseIllustList(String responseBody) {
    ...
    final list =
        (json['illusts'] as List?)?.cast<Map<String, dynamic>>() ??
