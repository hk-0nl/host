...
// ─── Danbooru source for Mangayomi ───────────────────────────────────────────
// API: https://danbooru.donmai.us/posts.json
//
// ── Bug fixes in this revision ───────────────────────────────────────────────
// FIX 1 — CF-popup on image load:
//   The CDN (cdn.donmai.us) gates images behind Referer / User-Agent headers.
...
//   Danbooru silently returns {"success":false,"message":"...more than 2 tags"}
...
class Danbooru extends MProvider {
  Danbooru({required this.source});
  ...
  // Danbooru API hard limits (per their documentation):
  ...
  Future<MPages> getPopular(int page) async {
    // order:score requires authentication on Danbooru. Try it first (for Gold+
    // users), then fall back to order:rank (hot) which works without login.
    final scored = await _fetchPage(
      page,
      query: "",
      sort: "order:score",
      rating: "",
  ...
  Future<MManga> getDetail(String url) async {
    ...
    if (isVideo) {
      ...
      if (fileUrl.isNotEmpty) {
        chapters.add(
          MChapter(name: _buildChapterLabel(post, ext), url: fileUrl),
        );
      }
      if (chapters.isEmpty) {
        chapters.add(
          MChapter(name: "Open Post", url: _base() + "/posts/" + id),
        );
      }
  ...
  // Referer to cdn.donmai.us, preventing the spurious CF-popup on 403.
  //
  ...
  Future<List<dynamic>> getPageList(String url) async {
    ...
    // Danbooru CDN requires Referer from the danbooru.donmai.us origin.
    return [
      {
        "url": url,
        "headers": {
          "Referer": _base() + "/",
          "User-Agent": "Mozilla/5.0 (compatible; Mangayomi-Danbooru/1.0)",
        },
      },
    ];
  }
  ...
  Future<MPages> _fetchPage(
    ...
    if (body.startsWith("{")) {
      ...
      try {
        ...
        if (obj is Map && obj["success"] == false) {
          ...
          throw Exception(
            "Danbooru: " + msg + " (composed query: '" + tagQuery + "')",
          );
        }
    ...
    for (final raw in posts) {
      ...
      final fileUrl = _resolveFileUrl(post);
      final previewUrl =
          post["preview_file_url"]?.toString() ?? _buildPreviewUrlFromMd5(post);
  ...
  String _resolveFileUrl(Map<String, dynamic> post) {
    ...
    // Reconstruct standard Danbooru CDN path from md5
    return _buildFileUrlFromMd5(post);
  }

  /// Danbooru CDN layout: /data/{md5[0..1]}/{md5[2..3]}/{md5}.{ext}
  ...
  /// Danbooru preview CDN layout: /data/preview/{md5[0..1]}/{md5[2..3]}/{md5}.jpg
  ...
  String _buildTitle(Map<String, dynamic> post) {
    ...
    final character = _tagString(
      post,
      "tag_string_character",
    ).split(" ").where((t) => t.isNotEmpty).take(2).join(", ");
  ...
  String _buildDescription(Map<String, dynamic> post) {
    ...
    if (score.isNotEmpty) {
      lines.add("Score: " + score + "  ↑" + (post["up_score"] ?? 0).toString());
    }
    ...
    if (character.isNotEmpty) {
      lines.add("Characters: " + character.replaceAll(" ", ", "));
    }
    final copyright = _tagString(post, "tag_string_copyright");
    if (copyright.isNotEmpty) {
      lines.add("Series: " + copyright.replaceAll(" ", ", "));
    }
  ...
  String _authParams() {
    ...
    return "&login=" +
        Uri.encodeQueryComponent(login) +
        "&api_key=" +
  ...
  List<dynamic> getSourcePreferences() {
    return [
      EditTextPreference(
        ...
        title: "Base URL",
        summary:
            "e.g. https://danbooru.donmai.us — change to use Safebooru or other clones",
      ...
      EditTextPreference(
        ...
        title: "Login",
        summary:
            "Danbooru username. Required to lift the 2-tag limit to 6 tags, "
      ...
      EditTextPreference(
        key: "api_key",
        title: "API Key",
        summary: "Found at https://danbooru.donmai.us/profile under API key.",
        value: "",
        dialogTitle: "API Key",
        dialogMessage: "",
      ),
