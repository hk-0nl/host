...
class Shikimori extends MProvider {
  ...
  Future<MManga> getDetail(String url) async {
    ...
    item.description = buildDescription(
      anime,
      asList(decodeJson(screenshotsPayload,   "decode screenshots payload",    "animes/" + animeId + "/screenshots")),
      asList(decodeJson(externalLinksPayload, "decode external links payload", "animes/" + animeId + "/external_links")),
      asList(decodeJson(videosPayload,        "decode videos payload",         "animes/" + animeId + "/videos")),
    );

    final chapters = <MChapter>[];

    for (final videoValue in asList(decodeJson(videosPayload, "decode videos payload", "animes/" + animeId + "/videos"))) {
      final video     = asMap(videoValue);
      final playerUrl = video["player_url"]?.toString() ?? video["url"]?.toString() ?? "";
      if (playerUrl.isEmpty) continue;

      final labelParts = <String>[
        "Video",
        if ((video["kind"]?.toString()    ?? "").isNotEmpty) video["kind"].toString(),
        if ((video["hosting"]?.toString() ?? "").isNotEmpty) video["hosting"].toString(),
        if ((video["name"]?.toString()    ?? "").isNotEmpty) video["name"].toString(),
      ];
      chapters.add(MChapter(name: labelParts.join(" | "), url: playerUrl));
    }

    for (final linkValue in asList(decodeJson(externalLinksPayload, "decode external links payload", "animes/" + animeId + "/external_links"))) {
      ...
      final labelParts = <String>[
        "External",
        if ((link["source"]?.toString() ?? "").isNotEmpty) link["source"].toString(),
        if ((link["kind"]?.toString()   ?? "").isNotEmpty) link["kind"].toString(),
      ];
  ...
  MPages toPages(List<dynamic> entries) {
    ...
    for (final entry in entries) {
      ...
      items.add(
        MManga()
          ..name     = name
          // Concatenation — dart_eval does not support string interpolation ($var / ${expr})
  ...
  String buildDescription(
    ...
    if (screenshots.isNotEmpty) {
      final sample = screenshots
          .take(5)
          .map((e) => resolveImageUrl(asMap(e)["preview"]))
  ...
  void addLine(List<String> lines, String label, String? value) {
    ...
    // Concatenation — dart_eval does not support "$label: $value" interpolation
  ...
  String joinNames(List<dynamic> values) {
    return values
        .map((v) => asMap(v))
        .map((v) => v["russian"]?.toString().trim().isNotEmpty == true
            ? v["russian"].toString()
            : v["name"]?.toString() ?? "")
        .where((v) => v.isNotEmpty)
        .join(", ");
  ...
  String stripHtml(String html) {
    ...
    // Concatenation — dart_eval does not support "<body>$html</body>" interpolation
  ...
  String resolveImageUrl(dynamic rawValue) {
    ...
    // Concatenation — dart_eval does not support "${getBaseUrl()}$value" interpolation
  ...
  String normalizeUrl(String url) {
    ...
    // Concatenation — dart_eval does not support "${getBaseUrl()}${...}" interpolation
    return getBaseUrl() + getUrlWithoutDomain(url);
  }

  String getOrderPreference({required String fallback}) {
    final value = getPreferenceValue(source.id, "preferred_order")?.toString().trim();
    return (value == null || value.isEmpty) ? fallback : value;
  }

  String getKindPreference() {
    final value = getPreferenceValue(source.id, "preferred_kind")?.toString().trim() ?? "any";
    return value == "any" ? "" : value;
  }

  String getStatusPreference() {
    final value = getPreferenceValue(source.id, "preferred_status")?.toString().trim() ?? "any";
    return value == "any" ? "" : value;
  }

  String getRatingPreference() {
    final value = getPreferenceValue(source.id, "preferred_rating")?.toString().trim() ?? "any";
    return value == "any" ? "" : value;
  }

  String getCensoredPreference() {
    return getPreferenceValue(source.id, "preferred_censored")?.toString().trim() ?? "any";
  }
  ...
  String getBaseUrl() {
    final value = getPreferenceValue(source.id, "domain_url")?.toString().trim();
    if (value == null || value.isEmpty) return source.baseUrl;
  ...
  String getApiBaseUrl() {
    ...
    if (value == null || value.isEmpty) {
      // Concatenation — dart_eval does not support "${source.baseUrl}/api" interpolation
      return source.baseUrl + "/api";
    }
    return value.endsWith("/") ? value.substring(0, value.length - 1) : value;
  }

  Future<String> apiGet(String path, {Map<String, String>? queryParameters}) async {
    // Concatenation — dart_eval does not support "${getApiBaseUrl()}/$path" interpolation
    final uri = Uri.parse(buildUrlWithQuery(getApiBaseUrl() + "/" + path, queryParameters));
  ...
  dynamic decodeJson(String body, String context, String path) {
    try {
      ...
      // Concatenation — dart_eval does not support "$context returned invalid JSON" interpolation
      fail(context + " returned invalid JSON", error: error, url: path);
    }
  }

  /// Build query string without using .entries — dart_eval does not implement
  /// the Map.entries getter, causing "entries method undefined" crashes.
  /// Use .keys iteration instead.
  String buildUrlWithQuery(String baseUrl, Map<String, String>? queryParameters) {
    if (queryParameters == null || queryParameters.isEmpty) return baseUrl;

    final parts = <String>[];
    for (final key in queryParameters.keys) {
      final value = queryParameters[key] ?? "";
      if (value.isNotEmpty) {
        // Concatenation inside loop — no string interpolation
        parts.add(Uri.encodeQueryComponent(key) + "=" + Uri.encodeQueryComponent(value));
      }
    }

    final filtered = parts.join("&");
  ...
  String mergeQueryParameters(String url, Map<String, String> updates) {
    ...
    if (queryString.isNotEmpty) {
      for (final pair in queryString.split("&")) {
        final parts = pair.split("=");
        if (parts.isNotEmpty) {
          params[Uri.decodeQueryComponent(parts[0])] =
              parts.length > 1 ? Uri.decodeQueryComponent(parts[1]) : "";
        }
      }
    }

    // Use .keys instead of .entries — dart_eval compatibility
  ...
  String stripFragmentAndQuery(String url) {
    final f = url.indexOf("#");
    final withoutFragment = f == -1 ? url : url.substring(0, f);
    final q = withoutFragment.indexOf("?");
    return q == -1 ? withoutFragment : withoutFragment.substring(0, q);
  }

  Never fail(String context, {Object? error, String? url}) {
    // Concatenation — dart_eval does not support "Shikimori: $context" interpolation
  ...
  List<dynamic> getSourcePreferences() {
    return [
      EditTextPreference(
        key: "domain_url", title: "Edit URL", summary: "",
        value: source.baseUrl, dialogTitle: "URL", dialogMessage: "",
      ),
      EditTextPreference(
        key: "api_url", title: "API URL", summary: "",
        value: source.baseUrl + "/api", dialogTitle: "API URL", dialogMessage: "",
      ),
      ListPreference(
        key: "preferred_order", title: "Default Order", summary: "", valueIndex: 0,
        entries:     ["Popularity","Ranked","Aired On","Episodes","Name","Created At"],
        entryValues: ["popularity","ranked","aired_on","episodes","name","created_at"],
      ),
      ListPreference(
        key: "preferred_censored", title: "Censored Filter", summary: "", valueIndex: 0,
        entries:     ["Any","Censored Only","Uncensored Only"],
        entryValues: ["any","true","false"],
      ),
      ListPreference(
        key: "preferred_kind", title: "Preferred Kind", summary: "", valueIndex: 0,
        entries:     ["Any","TV","Movie","OVA","ONA","Special","Music"],
        entryValues: ["any","tv","movie","ova","ona","special","music"],
      ),
      ListPreference(
        key: "preferred_status", title: "Preferred Status", summary: "", valueIndex: 0,
        entries:     ["Any","Announced","Ongoing","Released"],
        entryValues: ["any","anons","ongoing","released"],
      ),
      ListPreference(
        key: "preferred_rating", title: "Preferred Rating", summary: "", valueIndex: 0,
