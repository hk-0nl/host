...
class Sakugabooru extends MProvider {
  ...
  Future<MManga> getDetail(String url) async {
    ...
    if (isVideo) {
      ...
      if (chapters.isEmpty) {
        chapters.add(
          MChapter(name: "Open Post", url: _base() + "/post/show/" + id),
        );
      }
    ...
    if (body.startsWith("{")) {
      ...
      try {
        ...
        if (obj is Map && obj["success"] == false) {
          ...
          throw Exception(
            "Sakugabooru: " + msg + " (composed query: '" + tagQuery + "')",
          );
        }
  ...
  Future<Map<String, dynamic>> _fetchPost(String id) async {
    ...
    final originalFile =
        document.selectFirst("a#highres-show")?.attr("href") ??
    ...
    for (final li in document.select("ul#tag-sidebar li")) {
      ...
      final a = li
          .select("a")
          .where((e) => e.attr("href").contains("tags="))
          .lastOrNull;
      if (a != null) {
    ...
    map["score"] =
        document.selectFirst("span#post-score-" + id)?.text.trim() ?? "";
  ...
  List<dynamic> getSourcePreferences() {
    return [
      EditTextPreference(
        ...
        title: "Base URL",
        summary:
            "e.g. https://Sakugabooru.sakugabooru.com — change to use Safebooru or other clones",
      ...
      EditTextPreference(
        ...
        title: "Login",
        summary:
            "Sakugabooru username. Required to lift the 2-tag limit to 6 tags, "
      ...
      EditTextPreference(
        ...
        title: "API Key",
        summary:
            "Found at https://Sakugabooru.sakugabooru.com/profile under API key.",
        value: "",
        dialogTitle: "API Key",
        dialogMessage: "",
      ),
