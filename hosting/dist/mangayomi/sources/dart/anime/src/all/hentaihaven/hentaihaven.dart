...
class HentaiHaven extends MProvider {
  ...
  Future<MManga> getDetail(String url) async {
    ...
    final hentaiNameForEpisode =
        data["post_name"]?.toString() ?? data["name"]?.toString() ?? "";
    final episodes = asList(data["post_episodes"]);

    for (final entry in episodes) {
      final episode = asMap(entry);
      final episodeId =
          episode["chapter_id"]?.toString() ?? episode["id"]?.toString() ?? "";
      final episodeName =
      ...
      chapters.add(
        MChapter(name: episodeName, url: chapterUrl, dateUpload: episodeDate),
      );
    }
  ...
  Future<List<MVideo>> getVideoList(String url) async {
    ...
    final pageRes = await _safeGet(pageUrl, {
      "User-Agent": getUserAgent(),
      "Accept": "text/html,application/xhtml+xml,*/*",
      "Accept-Language": "en-US,en;q=0.9",
      "Connection": "close",
    });
    // Note: the Mangayomi bridge manages cookie persistence internally.
    // Manually accessing Response.headers causes a type mismatch in dart_eval.
    // The session cookies are handled transparently by the Client instance.
    ...
    if (streamUrl.isEmpty) {
      final pageRes2 = await _safeGet(pageUrl, {
        "User-Agent": getUserAgent(),
    ...
    final isDirect = streamUrl.endsWith(".m3u8") || streamUrl.endsWith(".mp4");
    if (isDirect) {
    ...
    String iframeBody = "";
    final iframeRes = await _safeGet(streamUrl, {
      "User-Agent": getUserAgent(),
      "Referer": baseOrigin,
      "Origin": baseOrigin,
      "Cookie": sessionCookies,
    ...
    if (iframeRes != null) {
      ...
      // Note: _extractCookies is not called here because Response.headers
      // is incompatible with the bridge's type system. The Client manages
      // cookie state internally between requests.
    ...
    if (m3u8.isEmpty && mp4.isEmpty) {
      try {
        ...
        iframeBody =
            await evaluateJavascriptViaWebview(streamUrl, script) ?? "";
  ...
  Future<dynamic> _safeGet(String url, Map<String, String> headers) async {
    try {
  ...
  MManga? mangaFromPartial(Map<String, dynamic> item) {
    ...
    final name =
        item["name"]?.toString() ?? item["post_name"]?.toString() ?? "";
    final title =
        item["title"]?.toString() ?? item["post_title"]?.toString() ?? "";
  ...
  String buildDescription(Map<String, dynamic> data) {
    ...
    addDetail(
      details,
      "Rating",
      buildRatingLine(asMap(data["post_rating"] ?? data["rating"])),
    );
    addDetail(
      details,
      "Tags",
  ...
  String extractIframeSrc(String html) {
    ...
    final endIdx = html.indexOf(delimChar == 34 ? '"' : "'", valueStart);
    if (endIdx == -1) return "";
