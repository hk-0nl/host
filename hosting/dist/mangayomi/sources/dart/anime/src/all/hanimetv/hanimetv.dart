...
// Hanime.tv — Anime source for hk-0nl registry
// dart_eval hardened: no string interpolation, no Map.entries, Connection:close on all requests
// Uses web2 X-Signature bypass to reach the v8 API without Cloudflare issues
...
class HanimeTv extends MProvider {
  ...
  Future<List<MVideo>> getVideoList(String url) async {
    ...
    for (final serverValue in servers) {
      ...
      for (final streamValue in asList(server["streams"])) {
        ...
        // Build quality label without string interpolation
        final qualityParts = <String>[];
        if (height.isNotEmpty) {
          qualityParts.add(height + "p");
        }
        if (width.isNotEmpty) {
          qualityParts.add(width);
        }
        if (kind.isNotEmpty) {
          qualityParts.add(kind);
        }
        qualityParts.add(serverName);
        final qualityLabel = qualityParts.join(" ").trim();
  ...
  List<MChapter> buildEpisodeChapters(List<dynamic> entries) {
    ...
    for (final entry in entries) {
      ...
      final slug = item["slug"]?.toString() ?? "";
      final name = item["name"]?.toString() ?? "";
      if (slug.isEmpty || name.isEmpty || seen[slug] == true) {
        continue;
      }
      seen[slug] = true;
      chapters.add(
  ...
  // No string interpolation — concatenate label and value with "+"
  ...
  // No string interpolation — pure concatenation
  ...
  // Rewrote to use .keys loop — Map.entries is banned in dart_eval
